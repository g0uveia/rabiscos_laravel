...
class PostsController extends Controller
{
    ...
    public function getLikes(Request $request) {
        ...
        foreach ($post->likes as $like) {
            $user_list[] = ['username' => $like->user->username, 'url' => route('user', ['id' => $like->user]), 'img_path' => Storage::url($like->user->img_path)];
        }

        if ($num_likes > 0) {
            header('Content-Type: application/json');
            return json_encode(['num_likes' => $num_likes, 'user_list' => $user_list]);
        }
